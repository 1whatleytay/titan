...
fn consume_eqv<'a, 'b>(
    ...
    let value = iter
        .collect_without(|token| &token.kind == &NewLine)
        .into_iter()
...
fn consume_macro<'a>(iter: &mut TokenCursor<'a, '_>) -> Result<Macro<'a>, PreprocessorReason> {
    let Some(symbol) = iter.next_adjacent() else {
        return Err(EndOfFile);
    };
    ...
    let mut stop = false;
    while !stop {
        let mut items = iter.collect_until(is_adjacent);

        let last = items.last().ok_or(EndOfFile)?;

        stop = match &last.kind {
            Symbol(name) => {
                let mut post_items = iter.collect_until(is_adjacent);
...
fn preprocess_cached<'a, P: TokenProvider<'a>>(
    ...
    let mut iter = TokenCursor::new(items, TokenInsights);
