...
impl Instruction {
    ...
    pub fn describe_trap_error(&self, registers: &dyn Registers) -> Option<TrapErrorDescription> {
        Some(match self {
            ...
            Instruction::Madd { s, t, .. } | Instruction::Msub { s, t, .. } => {
...
impl MemoryErrorDescription {
    fn address(&self) -> u32 {
        (self.source.value as i32).wrapping_add(self.immediate as i16 as i32) as u32
    }
}
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self.reason {
            MemoryErrorReason::Unmapped => {
                writeln!(
                    f,
                    "Memory access to 0x{:08x} is prohibited,",
                    self.address()
                )?;
                writeln!(
                    f,
                    " > {} ({} + {} = 0x{:08x} is unmapped)",
                    self.instruction,
                    self.source.hex_string(),
                    sig(self.immediate),
                    self.address()
                )?;
                writeln!(
                    f,
                    "Double check to make sure you meant to access this location."
                )
            }
            MemoryErrorReason::Alignment => {
                writeln!(
                    f,
                    "Memory access to 0x{:08x} must be a multiple of {} for this instruction.",
                    self.address(),
                    self.alignment
                )?;
                writeln!(
                    f,
                    " > {} ({} + {} = 0x{:08x} is not a multiple of {})",
                    self.instruction,
                    self.source.hex_string(),
                    sig(self.immediate),
                    self.address(),
    ...
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        match self.reason {
            OverflowAdd => {
                writeln!(
                    f,
                    "Encountered integer overflow during addition, which is prohibited."
                )?;
                writeln!(
                    f,
                    " > {} ({} + {} overflows)",
                    self.instruction,
                    self.source.signed_string(),
                    self.temp.signed_string()
                )?;
            }
            OverflowSub => {
                writeln!(
                    f,
                    "Encountered integer overflow during subtraction, which is prohibited."
                )?;
                writeln!(
                    f,
                    " > {} ({} - {} overflows)",
            ...
            OverflowOther => {
                writeln!(
                    f,
                    "Encountered integer overflow occurred, which is prohibited."
                )?;
                writeln!(
                    f,
                    " > {} ({} with {} overflows)",
